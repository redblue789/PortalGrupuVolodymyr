...
def manage_required(view_func):
    ...
    інших повертає на стрічку з поясненням, чому дія недоступна."""
    def wrapped(request, *args, **kwargs):
        if not can_manage_events(request.user):
            messages.error(
                request,
                'Додавати, редагувати чи видаляти події можуть лише '
                'адміністратори та модератори. Увійдіть під відповідним '
                'обліковим записом.'
            )
...
def calendar_view(request):
    ...
    # Нормалізація переходу через межі року
    if month < 1:
        month = 12
        year -= 1
    elif month > 12:
    ...
    cal = pycalendar.Calendar(firstweekday=0)  # 0 = понеділок
